...
const DeviceList = () => {
    ...
    const initialValues = {
      ...
      const {
        ...
        try {
            const response = await axiosClientPrivate.post('/devices', values);
       ...
       // alert(id);
        try {
          const response = await axiosClientPrivate.get(`/devices/${id}`);
            ...
            setFieldValue("deviceName",response.data.deviceName);
      ...
      const handleUpdate = async (id)=> {
       // alert(id);
        ...
        try{
             console.log(values);
             await axiosClientPrivate.put(`/devices/${id}`,update);
   ...
   const CustomActionComponent = ({id}) => {
    ...
    useEffect(() => {
        ...
        const getAllOhc = async () => {
            try {
                const response = await axiosClientPrivate.get('http://localhost:8080/devices?page=0&size=5', { signal: controller.signal });
    ...
    const exportpdf = async () => {
        ...
        const doc = new jsPDF();
        const header = [['Id', 'Device Name','Is Active']];
    ...
    return (
        ...
        <ToastContainer />
            ...
            <Popup showupdate={showupdate} id= {id} handleUpdate={handleUpdate} setShowupdate={setShowupdate} resetForm={resetForm} handleSubmit={handleSubmit}  openPopup={openPopup} setOpenPopup={setOpenPopup} title="Device Status">
